...
export type MascotState = 'idle' | 'typing' | 'password' | 'success' | 'error' | 'yawn';
...
export const Interactive3DBlob: React.FC<Interactive3DBlobProps> = ({
  ...
  useEffect(() => {
    if (activeState === 'password' || activeState === 'yawn') return;
    const interval = setInterval(() => {
      setBlink(true);
  ...
  useEffect(() => {
    if (activeState === 'password' || activeState === 'yawn') {
    ...
    >
      ...
      {showReactionBubble && (
        <motion.div
          ...
          className="mb-3 px-4 py-1.5 rounded-full bg-white/[0.09] border border-white/20 backdrop-blur-xl text-xs font-medium text-white shadow-[0_8px_25px_rgba(0,0,0,0.5)] flex items-center gap-1.5 text-center"
        >
      ...
      >
        ...
        >
          ...
          {/* Cheeks Blush */}
          <circle cx="88" cy="108" r="14" fill="url(#dreamCheekBlush)" />
          <circle cx="172" cy="108" r="14" fill="url(#dreamCheekBlush)" />
