...
export const JoinRoomPage: React.FC = () => {
  ...
  return (
    <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 pt-6 sm:pt-10 pb-28 sm:pb-36 space-y-12 sm:space-y-16">
      {/* Background Ambient Radial Glow */}
      <div className="absolute top-10 left-1/3 -translate-x-1/2 w-[750px] h-[350px] bg-gradient-to-r from-accent-blue/15 via-accent-purple/20 to-transparent blur-[140px] pointer-events-none -z-10" />
      <div className="absolute bottom-1/4 right-10 w-[550px] h-[320px] bg-accent-blue/10 blur-[140px] pointer-events-none -z-10" />
      ...
      {/* 1. ROOM CODE DIRECT JOIN: COMPLETELY UNBOXED (NO CARD) */}
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-accent-blue/15 text-accent-blue flex items-center justify-center border border-accent-blue/30 shadow-[0_0_20px_rgba(46,124,246,0.2)]">
            <KeyRound size={20} />
          </div>
          <div>
            ...
            <p className="text-xs sm:text-sm text-text-muted">
              Enter the 8-character secret invite code shared by the party host.
            </p>
          </div>
        </div>

        {error && (
          <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs sm:text-sm flex items-center gap-3 animate-fade-in max-w-2xl">
            <span className="w-2.5 h-2.5 rounded-full bg-red-400 animate-ping flex-shrink-0" />
            <span>{error}</span>
          </div>
        )}

        <form onSubmit={handleDirectJoin} className="max-w-3xl">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div className="relative flex-1">
              <input
                ...
                maxLength={10}
                className="w-full px-5 py-4 bg-white/[0.04] border border-white/[0.1] focus:border-accent-blue focus:ring-2 focus:ring-accent-blue/25 rounded-2xl font-mono text-base sm:text-lg tracking-widest text-white placeholder:text-text-muted/40 placeholder:font-sans placeholder:tracking-normal placeholder:text-sm focus:outline-none transition uppercase shadow-inner"
              />
              {directCode && (
                <button
                  type="button"
            ...
            <button
              ...
              className="px-8 py-4 rounded-2xl gradient-brand text-white font-bold text-sm sm:text-base shadow-[0_0_25px_rgba(46,124,246,0.3)] hover:brightness-110 active:scale-[0.99] disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center justify-center gap-2.5 group flex-shrink-0"
            >
          ...
          <div className="flex items-center gap-2 text-[11px] text-text-muted mt-2.5 px-1 font-mono">
            <span className="text-accent-blue">Tip:</span>
            <span>Codes are case-insensitive and can be entered with or without the dash (e.g. T6YZPOM2).</span>
          </div>
        </form>
      </div>
      ...
      {/* 2. PUBLIC WATCH PARTIES: BROWSE IN CARDS */}
